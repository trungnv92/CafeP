...
var HoTieuNoiDia = require('./model/HoTieuNoiDia');

router.get('/', function (req, res) {

    HoTieuNoiDia.find({}, function (err, listGia) {
...
cron.schedule('* * * * * *', function(){
    ...
    request('http://www.giatieu.com/', function (error, response, html) {
    if (!error && response.statusCode == 200) {
        var $ = cheerio.load(html);
        var json = [];
        $('#giatieu_sidebar tr').each(function (i, e) {
            //xóa hàng đầu tiên
            if(i != 0){
            json.push({
        ...
        for(var item in json){
            ...
            HoTieuNoiDia.update(conditions, newItem, options, 
                function(err, doc){
                    if(err){
